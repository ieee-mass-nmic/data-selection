...
PCU-Select avoids this search by assigning each example a conditional utility: its benefit for task $t$ when training uses configuration $p$. Its mean prediction approximates this utility:
\begin{equation}
  \widehat{\mu}_{\phi}(x,p,t)\approx u(x,p,t).
  \label{eq:surrogate}
\end{equation}
The target $u(x,p,t)\in[0,1]$ summarizes task-sketch loss reductions after a few PEFT update steps. PCU-Select rank-normalizes these reductions within groups that share the same PEFT, task, and measurement setting (see ``Method: PCU-Select''). Including $p$ allows the ranking to change with the trainable subspace; omitting $p$ would force every configuration to share one task-specific ranking. The task sketch $V_t$ identifies the desired task behavior, while $p$ identifies the updates available to the model. Utility is therefore relative to both the task and the PEFT configuration.
...
\begin{equation}
  \begin{aligned}
  C_{\mathrm{LESS}}(P,Q)
    &=P C^{\mathrm{data}}_{\mathrm{LESS}}+
      PQ C^{\mathrm{rank}}_{\mathrm{LESS}},\\
  C_{\mathrm{PCU}}(P,Q)
    &=C_{\mathrm{offline}}+PQ C^{\mathrm{pair}}_{\mathrm{PCU}}.
  \end{aligned}
  \label{eq:cost}
\end{equation}
Equation~\eqref{eq:cost} separates four costs. $C^{\mathrm{data}}_{\mathrm{LESS}}$ builds one PEFT-specific datastore, and $C^{\mathrm{rank}}_{\mathrm{LESS}}$ ranks it for one task. $C_{\mathrm{offline}}$ extracts reusable signals and trains PCU-Select once. $C^{\mathrm{pair}}_{\mathrm{PCU}}$ scores and selects data for one target pair. LESS builds $P$ datastores and performs $PQ$ rankings, whereas PCU-Select pays the offline cost once. The onboarding-cost experiment measures each cost term.

The objective is to retain per-PEFT LESS quality while replacing repeated datastore construction with shared offline work. Solving the two cost equations gives a break-even registry size; the onboarding-cost experiment substitutes the measured costs.
