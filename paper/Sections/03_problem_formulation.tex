...
Lower cost matters only if the selected subset preserves target-specific quality. We express the quality difference from per-PEFT LESS as
\begin{equation}
...
\end{equation}
where $\mathrm{Perf}_{\mathrm{PCU}}$ and $\mathrm{Perf}_{\mathrm{LESS}}$ denote their downstream scores and $\epsilon$ is the acceptable difference. The cost equations also identify when the shared offline investment pays off. For a fixed $Q$, PCU-Select costs less once $P$ exceeds
$P^{\star}=C_{\mathrm{offline}}/[C^{\mathrm{data}}_{\mathrm{LESS}}+
Q(C^{\mathrm{rank}}_{\mathrm{LESS}}-C^{\mathrm{pair}}_{\mathrm{PCU}})]$,
assuming the denominator is positive. The objective is therefore to retain per-PEFT LESS quality while replacing repeated datastore construction with shared offline work.

\paragraph{Design requirements.}
The formulation yields four design requirements. \textbf{PEFT conditioning} includes $p$ so the example ranking can change with the target configuration. \textbf{Task conditioning} uses $V_t$ so the model learns task-specific value instead of generic example quality. \textbf{Structured PEFT code} records where and how each PEFT updates the model in a form shared across families. \textbf{Reusable execution} moves gradient extraction, label construction, and scorer training offline, leaving cached-feature scoring for each new $(p,t)$ pair.
