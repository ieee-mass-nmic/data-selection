...
The main paper compares PCU-Select with per-PEFT LESS under an explicit quality
criterion. Let $\mathrm{Perf}_{\mathrm{PCU}}$ and
$\mathrm{Perf}_{\mathrm{LESS}}$ denote their downstream scores and $\epsilon$
...
For a fixed number of tasks $Q$, PCU-Select becomes cheaper when its shared
offline cost plus its per-pair cost falls below LESS's repeated datastore and
ranking costs. Solving the cost inequality from the main paper gives
\begin{equation}
  P > P^{\star}=
  ...
  \label{eq:break-even}
\end{equation}
provided that the denominator is positive. For the four-task workload,
$C^{\mathrm{pair}}_{\mathrm{PCU}}=1.51/4=0.3775$ GPU-hours, which the tables
display as 0.38. LESS costs 63.2 GPU-hours per configuration, while PCU-Select
pays 144.0 GPU-hours offline plus 1.51 GPU-hours per configuration. Therefore,
\begin{equation}
  P^{\star}=\frac{144.0}{63.2-1.51}=2.33.
  \label{eq:break-even-measured}
\end{equation}
PCU-Select costs less from the third integer-valued configuration onward. With
500 reduced calibration labels for each of four tasks, its marginal cost becomes
$1.51+4(1.05)=5.71$ GPU-hours and the worst-case threshold becomes
$144.0/(63.2-5.71)=2.50$ configurations.

\subsection{Per-Task Results Behind the Main Table}
\label{sec:appendix-per-task}

Table~1 in the main paper summarizes four task-native metrics after scaling
...
interval by resampling the evaluated cells. Parenthetical counts report how many
of the five PEFT cells PCU-Select wins within each task.
...
aggregate gap in Table~1 of the main paper. PCU-Select and per-PEFT LESS are
...
L-r8-highlayers, PRE-l16, PT-l32, and LN---while the configuration-sensitivity
study uses the additional LoRA variants. All use $1000$ steps, a cosine
schedule, and $0.03$ warmup; learning rates vary by configuration.
...
feeds two heads. One predicts mean utility $\widehat{\mu}_{\phi}$; the other
predicts an example-dependent uncertainty scale $\widehat{\sigma}_{\phi}$,
with a softplus floor of $10^{-3}$.
...
L2-LN & unseen family; labels available & calibrate before use \\
L2-Prompt & Prefix/P-Tuning & calibrate before use \\
\bottomrule
\end{tabular}
\caption{Deployment protocol for each registry-defined support level. Costs are per PEFT--task pair, in addition to 0.38 GPU-hours for online selection.}
\label{tab:protocol}
\end{table}
...
a tunable trade-off. Table~\ref{tab:pool-scaling} separates the online timing
components. At 300K, the scorer and clustering timers sum to 0.360 GPU-hours;
the revised 0.3775 total leaves a 0.0175 residual that the available phase logs
do not attribute. We report this residual separately instead of assigning it to
either phase. Across pool sizes, the end-to-end cost remains approximately
$170\times$ below the scaled per-PEFT LESS datastore cost.
...
\ENDIF
\STATE $(\widehat{\mu}_{\phi},\widehat{\sigma}_{\phi})\gets s_\phi(z_x,z_{p^{*}},z_{t^{*}})$; $q(x)\gets\widehat{\mu}_{\phi}-\lambda\widehat{\sigma}_{\phi}$ for all $x$
...
Table~\ref{tab:ood-levels} is the paired source for Figure~3 in the main paper.
Near-support targets transfer zero-shot.
Calibration closes most of the gap for far same-family targets and LN Tuning.
For Prefix/P-Tuning, the point estimate falls from 6.97 to 1.99, leaving a
stronger transfer boundary. The 0.18 cal-200-to-cal-500 change is descriptive
rather than evidence of a reliable difference between the two label budgets.
