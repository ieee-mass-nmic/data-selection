...
> Some of these configurations are in SEEN and some are in UNSEEN-config; E4 simultaneously serves two purposes: "configuration sensitivity" and "ID interpolation to unseen configurations."
...
| ----------------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- | ---------------------------------------------------- |
| **L0 ID interpolation** | UNSEEN-config configurations whose rank/placement fall within the SEEN convex hull, e.g., `L-r32-qkvo`, `AD-b16` | Family seen, concrete configuration unseen        | Direct zero-shot scoring                             |
| **L1 ID extrapolation** | Extreme configurations (`L-r64-all`, `AD-b256`, `L-r8-highlayers`)                                               | Family seen, configuration outside training range | zero-shot + calibration comparison                   |
| **L2 OOD family**       | `PRE-l16`, `PT-l32`, `BF` (prefix/ptuning/bitfit)                                                                | Family completely unseen                          | Calibration mode (see note below for prefix/ptuning) |
...
1. Use Mahalanobis `d²(p*)` to determine whether a configuration is OOD (threshold = 95th percentile of SEEN configuration `d²`); report the `d²` of every test configuration and whether calibration is triggered.
...
* **L0**: zero-shot is already close to per-PEFT LESS and clearly better than Random/RDS+ → proves that `z_p` supports ID interpolation.
...
* Required **`d²` vs performance degradation scatter plot**: validates whether Mahalanobis distance can predict when calibration is needed, i.e., the effectiveness of the OOD detector itself.
...
| F8 | Scatter plot              | E5: Mahalanobis d² vs performance degradation                              |
