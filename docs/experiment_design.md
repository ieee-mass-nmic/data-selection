...
| `LN`      | ln_tuning | tune both per-block RMSNorm scales |

> Note: Prefix/P-Tuning/LN Tuning are not included in the main conclusion, and are used only for OOD generalization and failure cases (consistent with research plan §12.3). On Llama-2, `LN` trains the `input_layernorm` and `post_attention_layernorm` scale parameters in every Transformer block and excludes the final `model.norm`.
...
| **L2 unseen family** | `PRE-l16`, `PT-l32`, `LN` (prefix/ptuning/ln_tuning) | Family completely unseen | Calibration only where native labels exist |

> **Implementation boundary (consistent with implementation version §13.2)**: revised E5 requires the native high-fidelity backend to support `ln_tuning` in addition to lora / ia3 / adapter. Before revised E5 runs, the backend must attach, optimize, and restore the two per-block RMSNorm scale vectors for `LN`. `PRE-l16`(prefix) and `PT-l32`(ptuning) still cannot generate calibration labels, so they can only be evaluated zero-shot and should be reported as failure boundaries (§6.5). Calibration labels are generated by `scripts/experiments/build_calib_labels.py`.
